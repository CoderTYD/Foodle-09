...
@property(nonatomic,strong)UITextField *msgTextView;
...
//    int height = keyboardRect.size.height;
    self.sendView.frame=CGRectMake(0, self.view.frame.size.height-60-keyboardRect.size.height, self.view.frame.size.width, 60);
}
-(void)keyboardWillHide:(NSNotification*)notice{
    self.sendView.frame=CGRectMake(0, self.view.frame.size.height-60, self.view.frame.size.width, 60);
}
...
- (void)addSendView{
    UIView*sendView=[[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-60, self.view.frame.size.width, 60)];
    sendView.backgroundColor=[UIColor lightGrayColor];
    [self.view addSubview:sendView];
    self.sendView=sendView;
    UITextField*textView=[[UITextField alloc]initWithFrame:CGRectMake(40, 10, self.view.frame.size.width-120, 40)];
    ...
    sendButton.frame=CGRectMake(self.view.bounds.size.width-70, 10, 60, 40);
    ...
    picBtn.frame=CGRectMake(5, 14, 30, 30);
...
//                MessageCell*cell=[tableView dequeueReusableCellWithIdentifier:@"messageCell"];
                //        cell.textLabel.text=message.messageId;
                //        NSLog(@"收到的文字是 txt -- %@",txt);
                ChatTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:@"chat"];
                if (nil==cell) {
                    cell=[[ChatTableViewCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:@"chat" StyleInt:message.direction];
                }
                cell.chatString=[(EMTextMessageBody*)msgBody text];
//                        NSLog(@"%@=%@",cell.label.text,textBody.text);
                NSString*dateStr=[NSString stringWithFormat:@"%lld",message.localTime];
                NSDateFormatter *formatter = [[NSDateFormatter alloc] init] ;
                [formatter setDateFormat:@"yyyy年MM月dd日"];
                NSDate *date=[formatter dateFromString:dateStr];
                cell.date=date;
                cell.userImageView.image=[UIImage imageNamed:@"aed80e26.jpeg"];
                return cell;
            }else{
//                AlternativeMessageCell*cell=[tableView dequeueReusableCellWithIdentifier:alternative_Identifier];
...
//                        NSLog(@"%@=%@",cell.label.text,textBody.text);
                ...
                NSDate *date=[formatter dateFromString:dateStr];
                cell.date=date;
                cell.userImageView.image=[UIImage imageNamed:@"e9a32440.jpeg"];
                return cell;
            }
            break;
        }
        case EMMessageBodyTypeImage:{
            ...
            if (message.direction==EMMessageDirectionSend) {
                ...
                [cell layout];
                cell.picture.image=[UIImage imageWithContentsOfFile:body.localPath];
                return cell;
            }else{
